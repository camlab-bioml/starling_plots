...
range(filter(df2, cohort == 'Tonsil', Initial %in% c('KM', "PG"), type == 'Arcsinh: CF=5')[,"PS"])
range(filter(df2, cohort == 'Ali 2020', Initial %in% c('KM', "PG"), type == 'Arcsinh: CF=5')[,"PS"])
range(filter(df2, cohort == 'Jackson 2020', Initial %in% c('KM', "PG"), type == 'Arcsinh: CF=5')[,"PS"])
...
write.csv(filter(res2, Lambda == '1.0', Dist == 'Student-T', Cell == 'Yes', Relax == 'Yes'), 
          "/home/campbell/yulee/project/st/plot/final/fig3a.csv")
...
df %>%
  ggplot(aes(x = factor(larger_than30, level = c("1", ">1")), y = as.numeric(doublet_prob), fill = as.factor(larger_than30), color = as.factor(larger_than30))) +
  ...
  geom_signif(comparisons = list(c("1", ">1")), map_signif_level = TRUE, color = 'black', y_position = 1.5) +
  facet_grid(rows = glue("{type}") ~ glue("{cohort}")) + #glue("Dataset: {cohort}")
  theme_pubr(legend = 'none', base_size = 15)
ggsave("fig3a.pdf", width = 5.5, height = 7 * (2/3))
...
df <- mutate(df,
             scenario = case_when(
               X == "no share" ~ "Cell type not in cell neighbours",
               X == "share" ~ "Cell type in cell neighbours"
             ))

cols <- c(
  "Cell type not in cell neighbours" = "grey50",
  "Cell type in cell neighbours" = scales::muted("red")
)

group_by(df, scenario) |> 
  #filter(prob > quantile(prob, 0.98)) |> 
  ungroup() |> 
  ggplot(aes(x = scenario, y = prob)) +
  ...
  labs(y = "Probability of other cell type in \n segmentation error",
       x = "") +
  geom_signif(comparisons = list(c("Cell type not in cell neighbours", "Cell type in cell neighbours")), map_signif_level = TRUE) +
  theme(legend.position = "none") + #, , panel.spacing.x=unit(0.5, "lines")
  scale_color_manual(values = cols)
ggsave("fig3b.pdf", width = 6, height = 7 * (2/3))
...
df %>% 
  ...
  facet_grid(rows = glue("{type}") ~ glue("{cohort}")) +
  theme_pubr(legend = 'none', base_size = 15)
ggsave("fig3c.pdf", width = 5.5, height = 7 * (2/3))
